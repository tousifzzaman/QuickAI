    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item) => (
            <span onClick={() => setSelectedStyle(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-50 text-green-700' : 'text-gray-300 border-gray-300'}`} key={item}>{item}</span>
          ))}
        </div>
        ...
        <div className='my-6 flex items-center gap-2'>
          <label className='relative cursor-pointer'>
            ...
            <span className='absolute left-1 top-1 w-3 bg-white rounded-full transition peer-checked:translate-x-4'></span>
          </label>
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96'>
        ...
        </div>
        {
          ...
          <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
            ...
            <p>Enyer a topic and click "Generate Image" to get started</p>
          </div>
        </div>
          ) : (
